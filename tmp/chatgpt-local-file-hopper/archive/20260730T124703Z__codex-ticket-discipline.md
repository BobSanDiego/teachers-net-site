...
that file, copy the final version to
`/home/bobreap/projects/teachers-net-site/tmp/chatgpt-local-file-hopper/`,
preserve its repository-relative path, update that hopper's
`HOPPER-MANIFEST.txt`, and link the hopper directory and copied file in the
report. Do not copy non-Markdown website files under this default rule.
