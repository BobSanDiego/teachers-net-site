(() => {
  ...
  const formatLocalIsoDate = (value) => { const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value || ""); if (!match) return ""; const year=Number(match[1]), month=Number(match[2]), day=Number(match[3]); if(month<1||month>12||day<1||day>31) return ""; return `${["January","February","March","April","May","June","July","August","September","October","November","December"][month-1]} ${day}, ${year}`; };
  const renderStep2Preview = () => {
    const preview = document.querySelector("#step2-preview");
    if (!preview) return;
    const basics = wizardState.basics, title = basics.jobTitle.trim() || "Teacher position", details = [basics.employmentType, Array.isArray(basics.gradeLevels) ? basics.gradeLevels.join(", ") : basics.gradeLevels, Array.isArray(basics.subjectAreas) ? basics.subjectAreas.join(", ") : basics.subjectAreas].filter(Boolean), salary = basics.salaryVisibility === "Do not show" ? "" : [basics.salaryMinimum && `$${basics.salaryMinimum}`, basics.salaryMaximum && `– $${basics.salaryMaximum}`, basics.salaryType].filter(Boolean).join(" "), start = basics.startTiming === "Specific Date" && basics.specificStartDate ? formatLocalIsoDate(basics.specificStartDate) : basics.startTiming;
  ...
  // JC053 taxonomy is a draft recruiter view. Production Job Center must consume an admin-curated Core Terms view controlling approved terms, nesting, and order; the Core Terms editor is the likely authority to adapt later.
  const jc053Taxonomy = { gradeLevels: ["Early Learners", "Pre-K", "Transitional Kindergarten", "Kindergarten", "Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6", "Grade 7", "Grade 8", "Grade 9", "Grade 10", "Grade 11", "Grade 12", "Adult Education", "Higher Education"], subjectAreas: ["English Language Arts", "Algebra", "Biology", "History", "Music", "General / Multiple Subjects", "Special Education", "English Learners / ESL", "Spanish", "Computer Science", "Career Technical Education", "Physical Education / Health", "Counseling", "Reading / Literacy", "Library / Media"] };
  const closeTaxonomyMenus = (focus) => document.querySelectorAll(".step2-taxonomy-field.is-open").forEach((field) => { field.classList.remove("is-open"); const popup=field.querySelector(".step2-taxonomy-popup"); if (popup) popup.hidden=true; field.querySelector(".step2-multiselect-trigger")?.setAttribute("aria-expanded", "false"); if (focus) field.querySelector(".step2-multiselect-trigger")?.focus({ preventScroll: true }); });
  const initializeTaxonomyMultiSelects = (root) => root.querySelectorAll(".grade-level-field, .subject-area-field").forEach((field) => { if (field.dataset.taxonomyInitialized) return; const selectControl = field.querySelector("select[multiple]"); if (!selectControl) return; const isGrade = field.classList.contains("grade-level-field"), key = isGrade ? "gradeLevels" : "subjectAreas", labels = jc053Taxonomy[key]; selectControl.innerHTML = labels.map((label) => `<option value="${step4Escape(label)}">${step4Escape(label)}</option>`).join(""); selectControl.className = "step2-taxonomy-native-control"; selectControl.tabIndex = -1; selectControl.setAttribute("aria-hidden", "true"); const fieldId = selectControl.id, popupId = `${fieldId}-popup`, triggerId = `${fieldId}-trigger`, wrapper = document.createElement("div"), trigger = document.createElement("button"), text = document.createElement("span"), chips = document.createElement("div"), popup = document.createElement("div"); wrapper.className = "step2-taxonomy-field"; trigger.type = "button"; trigger.id = triggerId; trigger.className = "step2-multiselect-trigger"; trigger.setAttribute("aria-haspopup", "listbox"); trigger.setAttribute("aria-expanded", "false"); trigger.setAttribute("aria-controls", popupId); text.className = "step2-multiselect-placeholder"; text.textContent = isGrade ? "Select grade levels" : "Select subject areas"; trigger.append(text); chips.className = "step2-taxonomy-chips"; popup.id = popupId; popup.className = "step2-taxonomy-popup"; popup.hidden = true; popup.setAttribute("role", "listbox"); popup.setAttribute("aria-multiselectable", "true"); popup.setAttribute("aria-label", isGrade ? "Grade level options" : "Subject area options"); labels.forEach((label, index) => { const option = document.createElement("label"), input = document.createElement("input"); option.className = "step2-taxonomy-option"; input.type = "checkbox"; input.id = `${fieldId}-option-${index}`; input.value = label; option.append(input, document.createTextNode(label)); popup.append(option); }); const actions = document.createElement("div"), clear = document.createElement("button"); actions.className = "step2-taxonomy-actions"; clear.type = "button"; clear.className = "step2-taxonomy-clear"; clear.textContent = "Clear all"; actions.append(clear); popup.append(actions); wrapper.append(trigger, chips, popup); selectControl.after(wrapper); field.dataset.taxonomyInitialized = "true"; const update = () => { const values = [...selectControl.selectedOptions].map((option) => option.value); wizardState.basics[key] = values; text.textContent = values.length ? `${values.length} selected` : (isGrade ? "Select grade levels" : "Select subject areas"); chips.replaceChildren(...values.map((value) => { const chip = document.createElement("span"), remove = document.createElement("button"); chip.className = "step2-taxonomy-chip"; chip.append(document.createTextNode(value)); remove.type = "button"; remove.className = "step2-taxonomy-remove"; remove.setAttribute("aria-label", `Remove ${value}`); remove.textContent = "×"; remove.addEventListener("click", () => { const option = [...selectControl.options].find((item) => item.value === value); if (option) option.selected = false; update(); syncWizardStateFromControl(selectControl); renderStep2Preview(); }); chip.append(remove); return chip; })); popup.querySelectorAll("input").forEach((input) => { input.checked = values.includes(input.value); }); clear.hidden = values.length <= 1; }; const open = () => { closeTaxonomyMenus(); wrapper.classList.add("is-open"); popup.hidden = false; trigger.setAttribute("aria-expanded", "true"); }; trigger.addEventListener("click", () => wrapper.classList.contains("is-open") ? closeTaxonomyMenus() : open()); popup.querySelectorAll("input").forEach((input) => input.addEventListener("change", () => { const option = [...selectControl.options].find((item) => item.value === input.value); if (option) option.selected = input.checked; update(); syncWizardStateFromControl(selectControl); renderStep2Preview(); })); clear.addEventListener("click", () => { [...selectControl.options].forEach((option) => { option.selected = false; }); update(); syncWizardStateFromControl(selectControl); renderStep2Preview(); }); update(); });
  document.addEventListener("click", (event) => { if (!event.target.closest(".step2-taxonomy-field")) closeTaxonomyMenus(); });
  document.addEventListener("keydown", (event) => { if (event.key === "Escape" && document.querySelector(".step2-taxonomy-field.is-open")) { event.preventDefault(); closeTaxonomyMenus(true); } });
  const initializeTaxonomyTreeDraft = (root) => root.querySelectorAll(".grade-level-field, .subject-area-field").forEach((field) => {
    if (field.dataset.taxonomyInitialized) return;
    const selectControl = field.querySelector("select[multiple]"), grade = field.classList.contains("grade-level-field");
    if (!selectControl) return;
    const key = grade ? "gradeLevels" : "subjectAreas", tree = grade ? [{id:"early",label:"Early Childhood",children:["Early Learners","Pre-K","Transitional Kindergarten","Kindergarten"]},{id:"elementary",label:"Elementary",children:["Grade 1","Grade 2","Grade 3","Grade 4","Grade 5"]},{id:"middle",label:"Middle School",children:["Grade 6","Grade 7","Grade 8"]},{id:"high",label:"High School",children:["Grade 9","Grade 10","Grade 11","Grade 12"]},{id:"adult",label:"Adult Education"},{id:"higher",label:"Higher Education"}] : [{id:"ela",label:"English Language Arts"},{id:"math",label:"Mathematics",children:["Algebra"]},{id:"science",label:"Science",children:["Biology"]},{id:"social",label:"Social Studies",children:["History"]},{id:"art",label:"Art",children:["Crafts and such"]},{id:"music",label:"Music"},{id:"general",label:"General / Multiple Subjects"},{id:"special",label:"Special Education"},{id:"esl",label:"English Learners / ESL"},{id:"world",label:"World Languages",children:["Spanish"]},{id:"tech",label:"Technology",children:["Computer Science"]},{id:"cte",label:"Career Technical Education"},{id:"pe",label:"Physical Education / Health"},{id:"counseling",label:"Counseling"},{id:"reading",label:"Reading / Literacy"},{id:"library",label:"Library / Media"}];
    const state = wizardState.basics.__taxonomyDraft || (wizardState.basics.__taxonomyDraft = {}), saved = state[key] || (state[key] = {direct:[],overrides:[],expanded:[]}), nodes = [];
    const flatten = (items, parent = null, depth = 0) => items.forEach((item, index) => { const node = typeof item === "string" ? {id:`${key}-${item.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`,label:item,children:[]} : {...item,id:`${key}-${item.id}`,children:item.children || []}; node.parent=parent; node.depth=depth; nodes.push(node); if (node.children.length) flatten(node.children,node,depth+1); });
    flatten(tree); const byId = Object.fromEntries(nodes.map((node) => [node.id,node])); const descendants = (node) => nodes.filter((item) => item !== node && item.parent && (function has(item,parent){return item.parent===parent || (item.parent && has(item.parent,parent));})(item,node));
    const wrapper=document.createElement("div"), trigger=document.createElement("button"), text=document.createElement("span"), popup=document.createElement("div"), chips=document.createElement("div"), fieldId=selectControl.id;
    selectControl.innerHTML=""; selectControl.className="step2-taxonomy-native-control"; selectControl.tabIndex=-1; selectControl.setAttribute("aria-hidden","true"); wrapper.className="step2-taxonomy-field"; trigger.type="button"; trigger.id=`${fieldId}-trigger`; trigger.className="step2-multiselect-trigger"; trigger.setAttribute("aria-haspopup","tree"); trigger.setAttribute("aria-expanded","false"); text.className="step2-multiselect-placeholder"; text.textContent=grade?"Select grade levels":"Select subject areas"; trigger.append(text); popup.id=`${fieldId}-popup`; popup.className="step2-taxonomy-popup step2-taxonomy-tree-popup"; popup.hidden=true; popup.setAttribute("role","tree"); chips.className="step2-taxonomy-chips"; wrapper.append(trigger,chips,popup); selectControl.after(wrapper); field.dataset.taxonomyInitialized="true";
    const branchHas = (node, id) => node.id===id || descendants(node).some((item)=>item.id===id), isDirect=(node)=>saved.direct.includes(node.id), isOverride=(node)=>saved.overrides.includes(node.id), branchMixed=(node)=>descendants(node).some((item)=>isOverride(item)), inherited=(node)=>{let p=node.parent; while(p){if(isDirect(p)&&!branchMixed(p)&&!isOverride(node)) return true; p=p.parent;} return false;};
    const selected = (node) => { if(isOverride(node)) return false; if(isDirect(node)) return true; return node.parent ? selected(node.parent) : false; };
    const summary = () => nodes.filter((node)=>selected(node)&&!branchMixed(node)&&(!node.parent||!selected(node.parent)||branchMixed(node.parent))).map((node)=>node.label);
    const refresh = () => { const values=summary(); wizardState.basics[key]=values; selectControl.innerHTML=values.map((value)=>`<option value="${step4Escape(value)}" selected>${step4Escape(value)}</option>`).join(""); text.textContent=values.length?values.join(", "):(grade?"Select grade levels":"Select subject areas"); chips.replaceChildren(); popup.querySelectorAll("input[data-node-id]").forEach((input)=>{const node=byId[input.dataset.nodeId], stateValue=selected(node), muted=inherited(node)||(!isDirect(node)&&node.children.length&&stateValue&&!descendants(node).every((child)=>selected(child))); input.checked=stateValue; input.classList.toggle("is-inherited",muted); input.closest(".step2-taxonomy-row")?.classList.toggle("is-inherited",muted); input.closest(".step2-taxonomy-row")?.classList.toggle("is-mixed",branchMixed(node)||(!isDirect(node)&&stateValue&&!descendants(node).every((child)=>selected(child))));}); };
    const toggleNode = (node, checked) => { if(checked){saved.direct=[...new Set([...saved.direct,node.id])]; saved.overrides=saved.overrides.filter((id)=>!branchHas(node,id)); descendants(node).forEach((child)=>{if(!saved.direct.includes(child.id)) saved.overrides=saved.overrides.filter((id)=>id!==child.id);});} else {saved.direct=saved.direct.filter((id)=>id!==node.id); if(node.parent&&selected(node.parent)) saved.overrides=[...new Set([...saved.overrides,node.id])]; descendants(node).forEach((child)=>{saved.direct=saved.direct.filter((id)=>id!==child.id); saved.overrides=[...new Set([...saved.overrides,child.id])];});} refresh(); syncWizardStateFromControl(selectControl); renderStep2Preview(); };
    const renderTree = (items, parent) => items.forEach((item) => { const nodeId=`${key}-${typeof item==="string"?item.toLowerCase().replace(/[^a-z0-9]+/g,"-"):item.id}`, node=byId[nodeId], row=document.createElement("div"); row.className="step2-taxonomy-row"; row.dataset.nodeId=nodeId; row.style.setProperty("--tree-depth",node.depth); row.setAttribute("role","treeitem"); row.setAttribute("aria-level",String(node.depth+1)); let children=null; if(node.children.length){children=document.createElement("div"); children.className="step2-taxonomy-children"; children.hidden=true; renderTree(node.children,children);} if(node.children.length){const disclosure=document.createElement("button"); disclosure.type="button"; disclosure.className="step2-taxonomy-disclosure"; disclosure.textContent="▸"; disclosure.setAttribute("aria-expanded","false"); disclosure.addEventListener("click",(event)=>{event.preventDefault(); event.stopPropagation(); const open=!row.classList.toggle("is-expanded"); disclosure.setAttribute("aria-expanded",String(!open)); disclosure.textContent=open?"▸":"▾"; children.hidden=open;}); row.append(disclosure);} else {const spacer=document.createElement("span"); spacer.className="step2-taxonomy-disclosure-spacer"; row.append(spacer);} const label=document.createElement("label"), input=document.createElement("input"); input.type="checkbox"; input.dataset.nodeId=nodeId; input.setAttribute("aria-label",node.label); input.addEventListener("change",()=>{const stableNode=byId[input.dataset.nodeId], wasChecked=input.checked; toggleNode(stableNode,wasChecked); if(stableNode.children.length){row.classList.add("is-expanded"); children.hidden=false;}}); label.append(input,document.createTextNode(` ${node.label}`)); label.addEventListener("click",(event)=>{if(event.target===input)return; event.preventDefault(); input.click();}); row.append(label); parent.append(row); if(children) parent.append(children); });
    const toolbar=document.createElement("div"); toolbar.className="step2-taxonomy-tree-toolbar"; const expand=document.createElement("button"), collapse=document.createElement("button"); expand.type=collapse.type="button"; expand.textContent="Expand all"; collapse.textContent="Collapse all"; [expand,collapse].forEach((button)=>button.className="step2-taxonomy-tree-action"); toolbar.append(expand,document.createTextNode(" | "),collapse); popup.append(toolbar); renderTree(tree,popup);
    const setExpanded=(open)=>{popup.querySelectorAll(":scope > .step2-taxonomy-row").forEach((row)=>{const child=row.nextElementSibling, disclosure=row.querySelector(".step2-taxonomy-disclosure"); if(!child)return; child.hidden=!open; row.classList.toggle("is-expanded",open); if(disclosure){disclosure.setAttribute("aria-expanded",String(open)); disclosure.textContent=open?"▾":"▸";}}); saved.expanded=open?nodes.filter((node)=>node.children.length).map((node)=>node.id):[];}; expand.addEventListener("click",()=>setExpanded(true)); collapse.addEventListener("click",()=>setExpanded(false));
    const open=()=>{closeTaxonomyMenus(); wrapper.classList.add("is-open"); popup.hidden=false; trigger.setAttribute("aria-expanded","true");}; trigger.addEventListener("click",()=>wrapper.classList.contains("is-open")?closeTaxonomyMenus():open()); refresh();
  });
  const renderWizardShell = (config, panel, content) => {
    ...
    initializeTaxonomyTreeDraft(panel);
    primeWizardState();
