(() => {
  ...
  step3Content.addEventListener("click", (event) => {
    ...
    if (option) { const value=option.dataset.benefitOption, category=option.dataset.benefitCategory, catalog=step3Benefits[category] || [], index=catalog.indexOf(value); step3State.selectedBenefits.add(value); renderStep3Benefits(); scheduleStep3Preview(); const next=catalog.slice(index+1).find((candidate) => !step3State.selectedBenefits.has(candidate)) || catalog.slice(0,index).find((candidate) => !step3State.selectedBenefits.has(candidate)); const focusTarget=[...step3Content.querySelectorAll(`[data-benefit-option][data-benefit-category="${category}"]`)].find((candidate) => candidate.dataset.benefitOption === next) || step3Content.querySelector("[data-benefit-option]") || document.querySelector("[data-benefit-clear]"); focusTarget?.focus(); return; }
    if (remove) { const value=remove.dataset.benefitRemove; step3State.selectedBenefits.delete(value); renderStep3Benefits(); scheduleStep3Preview(); [...step3Content.querySelectorAll("[data-benefit-option]")].find((candidate) => candidate.dataset.benefitOption === value)?.focus(); return; }
    if (clear) { step3State.selectedBenefits.clear(); renderStep3Benefits(); scheduleStep3Preview(); clear.focus(); }
  });
