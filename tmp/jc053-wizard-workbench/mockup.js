(() => {
  ...
  const initializeTaxonomyTreeDraft = (root) => root.querySelectorAll(".grade-level-field, .subject-area-field").forEach((field) => {
    ...
    const renderTree = (items, parent) => items.forEach((item) => { const node=byId[`${key}-${typeof item==="string"?item.toLowerCase().replace(/[^a-z0-9]+/g,"-"):item.id}`], row=document.createElement("div"); row.className="step2-taxonomy-row"; row.style.setProperty("--tree-depth",node.depth); row.setAttribute("role","treeitem"); row.setAttribute("aria-level",String(node.depth+1)); if(node.children.length){const disclosure=document.createElement("button"); disclosure.type="button"; disclosure.className="step2-taxonomy-disclosure"; disclosure.textContent="▸"; disclosure.setAttribute("aria-expanded","false"); disclosure.addEventListener("click",(event)=>{event.stopPropagation(); const open=!row.classList.toggle("is-expanded"); disclosure.setAttribute("aria-expanded",String(!open)); disclosure.textContent=open?"▸":"▾"; row.nextElementSibling.hidden=open;}); row.append(disclosure);} else {const spacer=document.createElement("span"); spacer.className="step2-taxonomy-disclosure-spacer"; row.append(spacer);} const label=document.createElement("label"), input=document.createElement("input"); input.type="checkbox"; input.dataset.nodeId=node.id; input.setAttribute("aria-label",node.label); input.addEventListener("change",()=>{toggleNode(node,input.checked); if(node.children.length){row.classList.add("is-expanded"); row.nextElementSibling.hidden=false;}}); label.append(input,document.createTextNode(` ${node.label}`)); label.addEventListener("click",(event)=>{if(event.target===input)return; if(node.children.length){event.preventDefault(); row.querySelector(".step2-taxonomy-disclosure")?.click();}}); row.append(label); parent.append(row); if(node.children.length){const children=document.createElement("div"); children.className="step2-taxonomy-children"; children.hidden=true; renderTree(node.children,children); parent.append(children);} });
