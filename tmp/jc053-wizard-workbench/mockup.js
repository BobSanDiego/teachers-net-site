(() => {
  ...
  class NavbarDropdown {
    constructor(trigger,config){
      ...
      const icons={
        'My Jobs':'<svg viewBox="0 0 24 24" focusable="false"><rect x="4" y="7" width="16" height="13" rx="2"/><path d="M9 7V5.5A1.5 1.5 0 0 1 10.5 4h3A1.5 1.5 0 0 1 15 5.5V7M4 11h16M10 11v2h4v-2"/></svg>',
        'Post a Job':'<svg viewBox="0 0 24 24" focusable="false"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M12 8v8M8 12h8"/></svg>',
        'Schools / Jobsites':'<svg viewBox="0 0 24 24" focusable="false"><path d="m3 10 9-5 9 5M5 10v10M9 10v10M15 10v10M19 10v10M3 20h18M2 10h20"/></svg>'
      };
      config.items.forEach(item=>{const el=document.createElement(item.available?'a':'span'); el.className='tnet-navbar-dropdown-item'+(item.available?'':' is-unavailable'); el.setAttribute('role','menuitem'); if(item.available){el.href=item.href}else{el.setAttribute('aria-disabled','true')} const icon=document.createElement('span');icon.className='tnet-navbar-dropdown-item-icon';icon.setAttribute('aria-hidden','true');icon.innerHTML=icons[item.label]||'<svg viewBox="0 0 16 16" focusable="false"><rect x="2.5" y="2.5" width="11" height="11" rx="2"/><path d="M5 8h6M8 5v6"/></svg>'; const label=document.createElement('span');label.textContent=item.label;el.append(icon,label);if(item.badge){const badge=document.createElement('span');badge.className='tnet-navbar-dropdown-item-badge';badge.textContent=item.badge;el.append(badge)}else if(item.current){const status=document.createElement('span');status.className='tnet-navbar-dropdown-item-status';status.textContent='Current';el.append(status)}this.menu.append(el)});
