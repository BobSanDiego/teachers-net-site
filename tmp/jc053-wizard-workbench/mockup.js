(() => {
  ...
  const step4Sync = (focusTarget = null, showErrors = false) => {
    const method=step4Content.querySelector("#step4-method")?.value, mode=step4Content.querySelector("#step4-deadline-mode")?.value, contact=step4Content.querySelector("input[name=step4-contact]:checked")?.value;
    ...
    step4Content.querySelector(".step4-override-fields").hidden=contact!=="override"; contactEmail.required=false;
    ...
    const preview=step4Content.querySelector("#step4-preview"), instructionsText=instructions.value.trim(), activeDestination=method==="url"?step4UrlValid(url.value):method==="email"?step4EmailValid(email.value):Boolean(instructionsText), action=activeDestination&&method==="url"?"<button type=\"button\" class=\"button secondary step4-preview-action\">Apply on Employer Site</button>":activeDestination&&method==="email"?"<button type=\"button\" class=\"button secondary step4-preview-action\">Apply by Email</button>":"";
    const contactValues=["#step4-contact-name","#step4-contact-email","#step4-contact-phone"].map(id=>step4Content.querySelector(id)?.value.trim()).filter(Boolean), contactText=document.querySelector("#step4-hide-contact")?.checked?"<p class=\"step4-preview-note\">Contact details will not be displayed publicly.</p>":contact==="override"?(contactValues.length?`<p>Contact: ${step4Escape(contactValues.join(" · "))}</p>`:""):"<p>School / Jobsite default contact will be used.</p>";
    ...
    preview.innerHTML=`<div class="step3-compact-listing"><strong>${step4Escape(document.querySelector("#job-title-step2")?.value||"Teacher position")}</strong><span>LAUSD · Los Angeles, CA</span><p>${step4Escape(document.querySelector("#step3-summary")?.value||"Add a short summary to preview this listing.")}</p></div><h4>How to Apply</h4><p>${method?step4Escape(method==="instructions"?"Candidates will follow the instructions below.":"Candidates will be directed to the selected destination."):"Select an application method to preview how candidates will apply."}</p>${instructionsText?`<p>${step4Escape(instructionsText).replace(/\n/g,"<br>")}</p>`:""}${action}${deadline}<h4>Contact</h4>${contactText}${materials.length?`<h4>Application Materials</h4><p>${materials.map(step4Escape).join(", ")}</p>`:""}`;
