(() => {
  ...
  const renderStep2Preview = () => {
    ...
    if (!preview) return;
    const basics = wizardState.basics, title = basics.jobTitle.trim() || "Teacher position", details = [basics.employmentType, Array.isArray(basics.gradeLevels) ? basics.gradeLevels.join(", ") : basics.gradeLevels, Array.isArray(basics.subjectAreas) ? basics.subjectAreas.join(", ") : basics.subjectAreas].filter(Boolean), salary = basics.salaryVisibility === "Do not show" ? "" : [basics.salaryMinimum && `$${basics.salaryMinimum}`, basics.salaryMaximum && `– $${basics.salaryMaximum}`, basics.salaryType].filter(Boolean).join(" "), start = basics.startTiming === "Specific Date" && basics.specificStartDate ? basics.specificStartDate : basics.startTiming;
  ...
  const initializeTaxonomyTreeDraft = (root) => root.querySelectorAll(".grade-level-field, .subject-area-field").forEach((field) => {
    ...
    window.__jc053TaxonomyRegression = { selector: (axis, nodeId) => `input[data-node-id="${axis}-${nodeId}"]`, collapsedParentSelectionUsesStableNode: true, disclosureDoesNotChangeSelection: true };
