...
class CFM_Views_Admin
{
  ...
  private static function render_draft_editor(int $version_id): void
  {
    ...
    echo '<hr><h2>Compose View</h2><p class="cfm-views-workflow">Select from the Library → Add to View → Arrange → Save draft → Preview → Publish</p><p>Start with a blank composition. Core Terms Library entries remain canonical UUID references and are never copied or edited here.</p>';
  ...
  private static function render_workbench_assets(): void
  {
    ...
    echo '<script id="cfm-views-selection-scope-script">(function(){"use strict";var root=document.querySelector("[data-cfm-views-workbench]");if(!root){return;}function descendants(box){var row=box.closest("[data-cfm-views-term]"),tree=row.closest("[data-cfm-views-tree]"),uuid=row.dataset.uuid;return [].filter.call(tree.querySelectorAll("[data-cfm-views-term] [data-cfm-views-select]"),function(candidate){var current=candidate.closest("[data-cfm-views-term]"),parent=current.dataset.parent;while(parent){if(parent===uuid){return true;}var parentRow=tree.querySelector("[data-uuid=\""+CSS.escape(parent)+"\"]");parent=parentRow?parentRow.dataset.parent:"";}return false;});}function sync(tree){[].slice.call(tree.querySelectorAll("[data-cfm-views-term]")).reverse().forEach(function(row){var box=row.querySelector("[data-cfm-views-select]"),kids=descendants(box);if(kids.length){var checked=kids.filter(function(item){return item.checked;}).length;box.indeterminate=checked>0&&checked<kids.length;}});}root.addEventListener("click",function(event){var box=event.target.closest("[data-cfm-views-select]");if(!box){return;}var kids=descendants(box),tree=box.closest("[data-cfm-views-tree]"),all=kids.length&&kids.every(function(item){return item.checked;});if(kids.length){if(!box.checked&&all){kids.forEach(function(item){item.checked=false;});box.checked=true;}else{kids.forEach(function(item){item.checked=box.checked;});}}sync(tree);var count=root.querySelector("[data-cfm-views-selected-count]");if(count){count.textContent=root.querySelectorAll("[data-cfm-views-select]:checked").length+" selected";}});})();</script>';
  }
