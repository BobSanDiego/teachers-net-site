...
The experience begins before Employer Operations. A visitor may browse the Job
Center without employer authority. Employer work requires a WordPress account.
After login or registration, the user chooses the correct organization path:
...
signals are not applications, applicants, interviews, or hires. My Jobs is the
job-level source for the metric set; summary context may appear inside the
workspace only when complete, accurately scoped, and subordinate to operational
job management.
...
- Approval establishes the canonical employer and active membership used by the
  standard My Jobs operating workspace.
...
Teachers.Net shell. My Jobs is the primary operating workspace. Prior Dashboard
summary concepts are absorbed into My Jobs as notifications, attention states,
workflow guidance, summary context, and School / Job Site scope; they are not a
separate V1 operating destination. The same workspace serves one employer,
multiple related employers, and multiple unrelated employers; memberships and
granted capabilities determine which operations are available.
...
The approved left-navigation entries are All My Jobs, My Schools / Job Sites,
Add School / Job Site, and Manage Schools / Job Sites. Exactly one selection is
active. All My Jobs and a specific School / Job Site are mutually exclusive; in
All My Jobs mode no individual school is highlighted, and in single-school mode
All My Jobs is not highlighted.
...
The heading remains **Job Timeline** and must not be renamed Lifecycle or Job
Lifecycle. Timeline rows reserve a uniform minimum height and fixed three-line
timeline space; rows do not shrink when fewer lines are populated. Draft rows
still occupy the full row height.

For non-draft listings, the first timeline line combines timing/state and
expiration date, such as `51 days left - Expires Jun 30, 2025`, `3 days left -
Expires Jun 15, 2025`, `Expired - Expired Mar 15, 2025`, or `Closed - Expired
Mar 15, 2025`. Draft listings use the exception `Last updated May 10, 2025`.
Secondary lines retain approved posted/submitted and updated date formats. Do
not add placeholder text such as `No expiration` or `Posted --`.

Live listings remain green with no icon normally; Expiring Soon remains green
with an exclamation icon only; Awaiting Review uses orange text only; Draft uses
neutral treatment; Expired is red and may use an exclamation icon; Closed is
neutral with no urgency color. Do not use colored bullets, colored dots,
orange/red Expiring Soon treatment, whole-row red fills, or persistent red
treatment for historical Closed records.
...
### 3.3 One Employer Operations workspace

Every approved employer uses the same Employer Operations workspace, including a
previously imported employer after claim. There is no separate imported,
claimed, recruiter, or statistics-dashboard operating surface.

### 3.4 One My Jobs inventory

My Jobs is the one employer-facing job manager. Confirmations and
...
archive follow one rule set across My Jobs, public finder, detail, saved jobs,
alerts, application actions, communications, and structured data.
...
| Active Employer Membership | The authorization grant that permits the user to access that employer's My Jobs workspace and eligible job actions. |
...
13. **Workspace metric set:** Which complete, accurately scoped summary values
    belong in the My Jobs workspace without turning it into an analytics
    dashboard?
...
### 7.3 Employer Operations / My Jobs continuity

12. A single-employer member reaches the correct My Jobs workspace directly.
13. A multi-employer member selects only from active authorized employers.
14. Employer context persists through My Jobs, Post, Edit, Review,
...
16. Workspace counts are complete, scoped to the current context, and attention
    items route to the correct My Jobs filter or School / Job Site context.
...
41. My Jobs workspace totals are complete and accurately scoped.
42. Renewal and duplication do not merge source and new-job engagement.
43. My Jobs, confirmation, public finder, public detail, saved jobs, alerts,
    application actions, and structured data agree on whether a job is Live.
