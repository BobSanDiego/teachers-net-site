...
Git worktree is `/home/bobreap/projects/teachers-net-site`, with production
source in `wordpress/wp-content/plugins/tnet-jobs`. The former
`/home/bobreap/projects/teachers-net-jobcenter` workbench is archived reference
only. If the worktree does not match, stop and report the mismatch.
...
JC053-STEP1-RESOURCE-IDENTITY-OVERFLOW-DEFENSE engineering correction and
selected-resource responsive convergence are complete for the accepted Step 1
state. The final Display Name authoring maximum remains undecided pending
persistence, validation, legacy-data, and consumer inspection. The approximate
40-character direction is not implementation authority.

JC052 Manage Schools / Jobsites implementation is now present in the
production-owned `tnet-jobs` runtime at `/jobs/employer/schools/`. It uses the
existing employer membership, `resource_manage` capability, School / Jobsite
repositories, identity resolution, address validation, and authorized-resource
listing for shared catalog view, create, and ordinary edit by authorized Admin
and Recruiter members. Media editing and destructive/archive management remain
separately bounded.

`JC052-MANAGE-SCHOOLS-JOBSITES-BROWSER-CERTIFICATION` is now complete. MCP
authenticated verification confirmed the canonical Admin route and shared
catalog. The preceding authenticated Recruiter roundtrip verified shared
catalog visibility, disposable create, and ordinary edit; its fixture was
removed afterward. No media or destructive-management behavior is implied.
Certification cycle `260810101500` is complete and documentation commit
`769c39c` is pushed.

`JC053-STEP2-INTEGRATION` was attempted in cycle `260810131301` and correctly
blocked before implementation because the then-current field contract assigned
Benefits to Step 2 while current design and continuity authority assigned
Benefits to Step 3. That contradiction is resolved: Benefits belongs
exclusively to Step 3 Optional Fields for V1. `JC053-STEP2-INTEGRATION` is now
released as the immediate executable objective after the bounded governance
authority consolidation.

`JC053-GOVERNANCE-AUTHORITY-DRIFT-AUDIT` cycle `260810133514` confirmed suspect
authority drift in the migrated field contract: Starting Date / Start Timing had
been listed as a V1 deferral without supporting product authority, while
accepted Step 2 evidence kept it in Step 2 with default `Immediately`, options
`Immediately`, `Specific Date`, and `Flexible`, and conditional Start Date reveal.
The same audit confirmed Work Location default `Use School / Jobsite Location`
and Salary Visibility default `Show Salary` were accepted but under-documented,
and preserved Benefits as Step 3 only.
...
Starting Date remains Step 2 V1; Work Location defaults to the selected School /
Jobsite location with no blank ordinary placeholder; Salary Visibility defaults
to `Show Salary` with no blank ordinary placeholder; Benefits remains Step 3
only; Requirements / Qualifications is optional/recommended for matching; and
Short Summary is handled by the deterministic summary review/gate before Step 4
rather than by ordinary immediate requiredness. `JC053-STEP2-INTEGRATION` is the
immediate next executable objective after this convergence.
...
If Codex cannot retrieve the companion ChatGPT session, it must troubleshoot
the connection before asking for the ticket to be pasted. Confirm whether the
requested chat title is listed as `kind: chatgpt`; distinguish it from any
similarly titled local Codex thread; report unavailable sources or missing
matches; and only then request paste fallback if the ChatGPT source is still
not readable. Do not blame ticket formatting unless the ChatGPT source was
successfully read and the latest ticket block itself failed the executable
format rule.
...
### JC057 sourced-job authority

`docs/job-center/sourced-job-v1-authority-contract.md` is now the canonical V1
authority for source discovery, managed/unmanaged employer handling,
AUTO_PUBLISH / REVIEW_FIRST / DO_NOT_SOURCE, separate per-Admin notification
preferences, source provenance, deterministic source identity, and the
discovery-to-Job boundary. It authorizes no implementation. Discovery remains
separate from Job lifecycle; schema and service work must follow the contract.
...
### JC056 employer authority and capability boundary
...
The membership prerequisite was reconciled in cycle `260810161000`:
membership 154 is active `employer_admin` for jobman/user 316 and orphaned
membership 159 is inactive with historical evidence preserved. The canonical
`TNet_Jobs_Employer_Capability_Service` was then implemented and pushed in
`d53c4cc`. It owns Admin/Recruiter capability decisions, Admin-all /
Recruiter-own job scope, one-active-employer service enforcement, legacy-role
normalization, and managed-employer detection. The database uniqueness
hardening is now installed at schema version `0.9.10` through generated column
`active_employer_user_id` and unique index `uq_active_employer_user`.
Founding-employer provisional authoring and Employer affiliation approval are
complete for the current Step 2 prerequisite.
...
The employer capability foundation and reversible database hardening are now
implemented. Founding-employer and affiliation workflows remain separate.
...
- JC053 Job Posting Wizard Re-Convergence: finalized shell and controls,
  ...
  `docs/job-center/jc053-wizard-product-contract-v1.md`. Current Step 3
  direction is paste-first rich authoring with Job Description as the only
  immediate required narrative field, Short Summary handled by deterministic
  summary review/gate before Step 4 when needed, Requirements / Qualifications
  optional/recommended for matching, grouped Optional Fields, compact Benefits
  selection with inline empty-state teaching, and incremental populated-content
  preview.
