...
Screen 3 is COMPLETE/FROZEN for product acceptance. The next Profile/onboarding
boundary requires a separately authorized post-avatar public-identity/profile
step, expected to include the promised changeable Display Name. No Screen 4
contract is yet authorized: its exact public-identity fields, defaulting and
validation, visibility timing, skip/continuation behavior, and destination must
be decided in that future ticket. Do not regenerate or recurate the bank,
