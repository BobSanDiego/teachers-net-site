...
final class TNet_Community_Attachment {
    ...
    public static function validate(array $attachment): array {
        $source=(string)($attachment['source_kind']??''); $reference=(string)($attachment['source_reference']??''); if (($source==='local_fixture'&&!preg_match('/^fixture:[A-Za-z0-9_\/-]+$/',$reference))||($source==='local_upload'&&!preg_match('/^wp-content\/uploads\/[A-Za-z0-9_\/-]+$/',$reference))||!in_array($source,['local_fixture','local_upload'],true)) throw new InvalidArgumentException('ATTACHMENT_SOURCE_UNSUPPORTED');
