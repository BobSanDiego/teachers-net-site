...
final class TNet_Community_Thread_Controller {
    ...
    public static function render(): void {
        ...
        $root = $data['root'];
        $html = '<main><p class="thread-navigation"><a class="back-to-community" href="'.esc_url(home_url('/community/')).'">← Back to Community</a></p><h1>' . esc_html($root['title']) . '</h1><article class="thread-card"><p class="meta">' . esc_html($root['_author_display'] . ' · ' . $root['created_at']) . '</p><div>' . TNet_Community_Authoring::markdown($root['body']) . '</div>' . self::attachments($root) . TNet_Community_Link_Preview::render($root['preview'] ?? []) . '</article>';
    ...
    private static function reply_form_normalized(array $root, array $rows, array $errors): string {
        ...
        $body = '<textarea id="reply-body" name="body" rows="2" placeholder="Reply to '.esc_attr($root['_author_display']??'this author').'…" aria-label="Reply to '.esc_attr($root['_author_display']??'this author').'…" required></textarea><div class="reply-media-zone" id="reply-media-zone"><label class="reply-photo-action" for="reply-image-file">📷 Photo</label><input id="reply-image-file" name="image_file" type="file" accept="image/jpeg,image/png,image/webp" hidden><span class="reply-drop-hint">Paste or drop an image</span><div id="reply-image-preview" hidden></div><button type="button" class="secondary" id="reply-remove-image" hidden>Remove image</button><input id="attachment_alt" name="attachment_alt" type="hidden"></div>';
        $help = '<details><summary>Formatting help</summary><p>Use **bold**, *italic*, \`code\`, quotes, lists, or [links](https://example.com).</p></details>';
        $script = '<script>(function(){document.querySelectorAll("[data-reply-target]").forEach(function(a){a.addEventListener("click",function(){document.getElementById("reply-target").value=a.dataset.replyTarget;document.getElementById("reply-context").textContent="Replying to "+a.textContent;document.getElementById("reply-body").focus()})});var f=document.getElementById("reply-image-file"),z=document.getElementById("reply-media-zone"),p=document.getElementById("reply-image-preview"),r=document.getElementById("reply-remove-image"),u=null;function clear(){if(u)URL.revokeObjectURL(u);u=null;f.value="";p.hidden=true;p.innerHTML="";r.hidden=true}function stage(file){if(!file)return;if(!["image/jpeg","image/png","image/webp"].includes(file.type)||file.size>10485760){clear();return}if(u)URL.revokeObjectURL(u);u=URL.createObjectURL(file);try{var dt=new DataTransfer();dt.items.add(file);f.files=dt.files}catch(e){return}p.innerHTML="<img src=\\\""+u+"\\\" alt=\\\"\\\" style=\\\"max-width:100%;max-height:220px;border-radius:8px\\\"><p>Image ready</p>";p.hidden=false;r.hidden=false}f.addEventListener("change",function(){stage(f.files[0])});r.addEventListener("click",clear);["dragenter","dragover"].forEach(function(x){z.addEventListener(x,function(e){e.preventDefault();z.classList.add("active")})});["dragleave","drop"].forEach(function(x){z.addEventListener(x,function(e){e.preventDefault();z.classList.remove("active")})});z.addEventListener("drop",function(e){stage(e.dataTransfer.files[0])});document.getElementById("reply-body").addEventListener("paste",function(e){for(var item of (e.clipboardData?.items||[])){if(item.kind==="file"&&item.type.startsWith("image/")){e.preventDefault();stage(item.getAsFile());break}}});window.addEventListener("unload",function(){if(u)URL.revokeObjectURL(u)});})();</script>';
        $script = str_replace(chr(92).chr(92).'"', chr(92).'"', $script);
