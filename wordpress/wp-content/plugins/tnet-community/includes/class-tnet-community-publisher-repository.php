...
final class TNet_Community_Publisher_Repository {
    ...
    public function persist_publication_in_transaction(array $publication, array $actor = []): array {
        ...
        $post = TNet_Community_Canonical_Route::decorate_topic($post, $this);
        ...
        try {
            ...
            if (!empty($publication['media_items'])) (new TNet_Community_Media_Registry())->attach_in_transaction($post['post_id'], (array) $publication['media_items']);
    ...
    public function find_topic_by_canonical_slug(string $community_id, string $slug): ?array {
        global $wpdb;
        $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$this->tables['posts']} WHERE community_id=%s AND post_type='topic'", $community_id), ARRAY_A) ?: [];
        foreach ($rows as $row) {
            $decoded = $this->decode($row);
            $route = $decoded['compatibility_refs']['canonical_route'] ?? [];
            if (($route['thread_slug'] ?? '') === $slug) return $decoded;
        }
        return null;
    }
    public function list_latest_topics(int $limit=20, ?string $community_id=null): array { global $wpdb; $where="p.post_type='topic' AND p.publication_state IN ('published','restored')"; $params=[]; if($community_id!==null){$where.=' AND p.community_id=%s';$params[]=$community_id;} $sql="SELECT p.*, (SELECT COUNT(*) FROM {$this->tables['posts']} r WHERE r.thread_id=p.thread_id AND r.post_type='reply' AND r.publication_state IN ('published','restored')) AS reply_count, (SELECT MAX(a.updated_at) FROM {$this->tables['posts']} a WHERE a.thread_id=p.thread_id AND a.publication_state IN ('published','restored')) AS last_activity FROM {$this->tables['posts']} p WHERE {$where} ORDER BY last_activity DESC, p.created_at DESC, p.id DESC LIMIT %d"; $params[]=$limit; return array_map([$this,'decode'],$wpdb->get_results($wpdb->prepare($sql,...$params),ARRAY_A) ?: []); }
    ...
    public function transition_post(string $post_id,string $new_state,string $actor,string $reason): array { global $wpdb; $wpdb->query('START TRANSACTION'); try { $transition=$this->transition_post_in_transaction($post_id,$new_state,$actor,$reason); if (empty($transition['accepted'])) { $wpdb->query('ROLLBACK'); return $transition; } $wpdb->query('COMMIT'); return $transition; } catch (Throwable $error) { $wpdb->query('ROLLBACK'); return ['accepted'=>false,'reason_code'=>$error->getMessage()]; } }
    /** Apply a canonical publisher lifecycle transition inside a caller-owned transaction. */
    public function transition_post_in_transaction(string $post_id,string $new_state,string $actor,string $reason): array { global $wpdb; $post=$this->find_post($post_id); if (!$post) return ['accepted'=>false,'reason_code'=>'POST_NOT_FOUND']; $transition=(new TNet_Community_Publisher_Domain())->transition($post,$new_state,$actor,$reason); if (empty($transition['accepted'])) return $transition; $updated=$wpdb->update($this->tables['posts'],['publication_state'=>$new_state,'updated_at'=>current_time('mysql',true)],['post_id'=>$post_id],['%s','%s'],['%s']); $audit=$wpdb->insert($this->tables['audit'],['post_id'=>$post_id,'action'=>$reason,'actor_id'=>$actor,'reason'=>$reason,'previous_state'=>$post['publication_state'],'new_state'=>$new_state,'evidence_json'=>wp_json_encode($transition),'created_at'=>current_time('mysql',true)],array_fill(0,8,'%s')); if (false===$updated||false===$audit) throw new RuntimeException('LIFECYCLE_PERSISTENCE_FAILED'); return $transition; }
    ...
    private function decode(array $row): array { foreach(['compatibility_json','audit_json'] as $key) if (isset($row[$key])) $row[$key]=json_decode($row[$key],true) ?: []; if (isset($row['compatibility_json'])) { $row['compatibility_refs']=$row['compatibility_json']; $media=(new TNet_Community_Media_Registry())->attachments_for_post((string)($row['post_id']??'')); if ($media) { $row['compatibility_refs']['composer']['attachments']=$media; $row['compatibility_json']=$row['compatibility_refs']; } } if (isset($row['audit_json'])) $row['audit_metadata']=$row['audit_json']; return $row; }
}
