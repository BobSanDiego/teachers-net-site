...
async function stage(file){if(!valid(file)){message('Choose a JPEG, PNG, or WebP image up to 10 MB.');return}const objectUrl=URL.createObjectURL(file),item={id:crypto.randomUUID(),file,objectUrl,url:objectUrl};items.push(item);card(item);message('Preparing image upload…');sync();upload(item,false);}
