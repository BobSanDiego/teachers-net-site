...
final class TNet_Community_Topic_Composer_Controller {
    ...
    private static function submit(): void {
        ...
        $community = sanitize_key(wp_unslash($_POST['community_id'] ?? ''));
        ...
        if ($errors) self::form($errors, ['community_id' => $community, 'title' => $title, 'body' => $body]);

        $draft = [
            'submission_id' => 'web-' . wp_generate_uuid4(),
        ...
        if (empty($result['accepted']) || empty($result['post']['post_id'])) {
            self::form(['The topic could not be published. Please try again.'], ['community_id' => $community, 'title' => $title, 'body' => $body]);
        }
        wp_safe_redirect(home_url('/community/thread/' . rawurlencode($result['post']['post_id']) . '/'));
        exit;
    }
