...
function clear(){if(objectUrl)URL.revokeObjectURL(objectUrl);objectUrl=null;file.value='';const image=preview.querySelector('img');if(image)image.remove();remove.hidden=true;staged.classList.remove('has-image');status.textContent='Add an image, paste one, or drop one here.'}
