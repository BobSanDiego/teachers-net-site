...
final class TNet_Community_Topic_Composer_Controller {
    ...
    private static function form(array $errors, array $values=[]): void {
        ...
        $html .= self::embedded_form($community, $name, $back, compact('sid', 'body', 'choice', 'alt'));
    ...
    public static function embedded_form(string $community, string $name, string $cancel, array $values=[], bool $modal=false): string {
        $sid=$values['sid']??'web-'.wp_generate_uuid4(); $body=$values['body']??''; $choice=$values['choice']??'keep'; $alt=$values['alt']??'';
        $cancel_control=$modal ? '<button type="button" class="secondary" data-close-composer>Cancel</button>' : '<a href="'.esc_url($cancel).'">Cancel</a>';
        return '<form data-composer-view="scoped-topic" class="c3-topic-composer" method="post" enctype="multipart/form-data">'.wp_nonce_field('tnet_community_topic','tnet_topic_nonce',true,false).'<input type="hidden" name="submission_id" value="'.esc_attr($sid).'"><input type="hidden" name="community_id" value="'.esc_attr($community).'"><label class="screen-reader-text" for="body">Share with '.esc_html($name).'</label><textarea id="body" name="body" rows="8" placeholder="Share a thought, question, or resource with '.esc_attr($name).'…" required>'.esc_textarea($body).'</textarea><section id="staged-content" class="staged" aria-live="polite"><p id="image-status" class="status">Add an image, paste one, or drop one here.</p><div id="image-preview"></div><div id="dropzone" class="composer-media-actions"><button type="button" class="secondary" id="add-photo">Add Photo</button><input id="image_file" name="image_file" type="file" accept="image/jpeg,image/png,image/webp" hidden><span class="drop-hint">Paste or drop an image directly onto the post.</span></div><button type="button" class="secondary" id="remove-image" hidden>Remove image</button><input id="attachment_alt" name="attachment_alt" value="'.esc_attr($alt).'" type="hidden"><input id="preview_choice" name="preview_choice" value="'.esc_attr($choice).'" type="hidden"><div id="link-preview" class="preview" hidden></div></section><div class="actions"><button type="submit">Post</button>'.$cancel_control.'</div></form>'.self::script();
    }
