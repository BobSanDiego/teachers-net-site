...
final class TNet_Community_Publisher_Application {
    ...
    public function publish_and_persist(array $draft,array $communities,array $actor=[]): array { $result=$this->domain->publish($draft,$communities,(string)($draft['moderation_input']??'clear')); if (empty($result['accepted'])) return $result; $result['media_items']=(array)($draft['media_items']??[]); return (new TNet_Community_Publisher_Repository())->persist_publication($result,$actor); }
