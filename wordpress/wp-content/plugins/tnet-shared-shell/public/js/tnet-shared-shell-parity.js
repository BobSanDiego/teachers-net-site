(function () {
  ...
  function initLab(root) {
    ...
    root.querySelector('[data-shell-candidate-save]').addEventListener('click', function () { var name = (candidateName.value || '').trim(); if (!name) { announce('Name the candidate before saving.'); candidateName.focus(); return; } state.candidates = state.candidates || {}; state.candidates[name] = currentSettings(); safeWrite(state); refreshCandidates(); candidateSelect.value = name; announce(name + ' saved locally.'); });
    root.querySelector('[data-shell-candidate-load]').addEventListener('click', function () { var candidate = state.candidates && state.candidates[candidateSelect.value]; if (!candidate) { announce('Choose a saved candidate first.'); return; } settings = Object.assign({}, defaults, candidate); setPresentation(settings.presentation, true); candidateName.value = candidateSelect.value; announce(candidateSelect.value + ' loaded.'); });
    root.querySelector('[data-shell-candidate-delete]').addEventListener('click', function () { var name = candidateSelect.value; if (!name || !state.candidates || !state.candidates[name]) { announce('Choose a saved candidate first.'); return; } delete state.candidates[name]; safeWrite(state); refreshCandidates(); announce(name + ' deleted.'); });
    root.querySelector('[data-shell-candidate-reset]').addEventListener('click', function () { settings = Object.assign({}, defaults); setPresentation(defaults.presentation, true); announce('Baseline restored.'); });
    root.querySelector('[data-shell-settings-copy]').addEventListener('click', function () {
