...
final class TNet_Identity_Public {
  ...
  public static function render_avatar() {
    ...
    self::page(__('Add a profile photo', 'tnet-identity'), function () use ($errors, $saved, $avatar, $client_entries, $generation, $presentation) {
      ?>
      <main class="tnet-identity-card tnet-identity-card--avatar" aria-labelledby="tnet-identity-avatar-title">
        <?php if ($saved) : ?>
          ...
          <a class="tnet-identity-submit tnet-identity-submit--link" href="<?php echo esc_url(home_url('/jobs/')); ?>"><span>Continue to Teachers.Net</span><span class="tnet-identity-submit-arrow" aria-hidden="true">→</span></a>
        <?php else : ?>
